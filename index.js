...
const datos = JSON.parse(
  readFileSync(join(__dirname, "peliculas.json"), "utf-8"),
);
...
app.get("/", (req, res) => {
  res.json({
    ...
    endpoints: ["/api/peliculas", "/api/peliculas/:id", "/api/generos", "/api/peliculas&generos=accion"],
  });
});
...
app
  ...
  .on("error", (err) => {
    if (err.code === "EADDRINUSE") {
      console.error(
        `El puerto ${PORT} ya está en uso. Cierra el otro proceso o ejecuta: set PORT=3001 && npm start`,
      );
    } else {
